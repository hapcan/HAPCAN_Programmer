...
public class ProgrammingUniv4 : ProgrammingBase
{
    //FIELDS
    ...
    public override int Address
    {
        get
        {
            if (base.Address >= 0x000 && base.Address <= 0x03FF)        //eeprom real range 0x380000-0x3803FF
                return base.Address + 0x380000;
            else
                return base.Address;
        }
    }
    ...
    //CONSTRUCTOR
    public ProgrammingUniv4(HapcanNode node, Action ReportProgress) : base(node, ReportProgress)
    {
        _node = node;
        Data = new byte[0x20000];                          //eeprom   0x00000-0x003FF
        for (int i = 0; i < Data.Length; i++)              //firmware 0x02000-0x0FFFF
            Data[i] = 0xFF;                                //data     0x10000-0x1FFFF 
    }
    ...
    public override async Task SmartReadDataMemoryAsync(CancellationTokenSource cts)
    {
        ...
        //read flash
        if (flashTo > 0x10000)
            await ReadAsync(0x10000, flashTo, cts);
    ...
    public override async Task SmartWriteDataMemoryAsync(byte[] eepromBuffer, byte[] flashBuffer, CancellationTokenSource cts)
    {
        ...
        if (eepromBuffer.Length != 0x400 || flashBuffer.Length != 0x10000)
                ...
                if (_node.Flash[i] != flashBuffer[i])                   //any difference in block of 64 bytes?
                {
                    ...
                    for (int subadr = adr; subadr < adr + 64; subadr += 8)          //jump eeprom in 8byte blocks
                    {
                        for (int j = subadr; j < subadr + 8; j++)                 //check every byte in that block
                        {
                            ...
                            {
                                await WriteAsync(movedFlashBuffer, subadr + 0x10000, subadr + 0x10007, cts);  //save block of 8 bytes
